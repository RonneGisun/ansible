...
description:
    - This module allows the administration of Keycloak scope mappings through the Keycloak REST API. It
    ...
    clientroles:
        description:
            - A dict of lists of role representations (in dict-form) to be acted on. At minimum,
              C(name) is required in a role representation. See
              U(http://www.keycloak.org/docs-api/3.3/rest-api/index.html#_rolerepresentation) for
              more fields (though they should not be required).
              The dict's keys are the client-role scopes for the attached roles representations. There
              is an example below. Either C(roles) or C(clientroles) is
              required, depending on C(type).
